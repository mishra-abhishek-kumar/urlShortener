const { isAdmin, isUser, isAdminOrUser } = require("../middleware/authService");
...
class RouteConfig {
	...
	getAccessRole(routeItem) {
		let isAdminRoute = routeItem?.isAdminRoute || false;
		let isUserRoute = routeItem?.isUserRoute || false;
		return {
			isUserRoute,
			isAdminRoute,
		};
		...
		if (secured) {
			if (isAdmin && isUser) {
				application.route(route)[method]((req, res, next) => {
					isAdminOrUser(settingsConfig, req, res, next);
				});
			} else {
				if (isAdminRoute) {
					application.route(route)[method]((req, res, next) => {
	...
	registerRoutes(application, settingsConfig) {
		...
		for (let i = 0, { length } = config.routes; i < length; i += 1) {
			...
			const { isAdminRoute, isUserRoute } = this.getAccessRole(routeItem);

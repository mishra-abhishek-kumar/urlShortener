...
const roleBasedQueryInjector = async (settingsConfig, req, res, next) => {
	...
	logger.info(
		`[CHECK_PROSPECT_MIDDLEWARE] : Inside roleBasedQueryInjector middleware`
	);
	...
	try {
		...
		if (!user.roles) {
			return res
				...
				.json({ message: "Role not found for current user" });
		}
		...
		logger.info(
			`[CHECK_PROSPECT_MIDDLEWARE] : End of roleBasedQueryInjector middleware`
		);
		next();
	} catch (error) {
